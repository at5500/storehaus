...
impl CacheManager {
    ...
    fn build_record_key(&self, table_name: &str, id: &str) -> String {
        format!("{}:{}:record:{}", self.config.key_prefix, table_name, id)
    }

    /// Generate cache key for query results
    fn build_query_key(&self, table_name: &str, query_hash: &str) -> String {
        format!("{}:{}:query:{}", self.config.key_prefix, table_name, query_hash)
    }
    ...
    pub async fn get_record<T>(&self, table_name: &str, id: &str) -> Result<Option<T>, CacheError>
    where
        T: for<'de> Deserialize<'de>,
    {
        let cache_key = self.build_record_key(table_name, id);
    ...
    pub async fn set_record<T>(&self, table_name: &str, id: &str, value: &T) -> Result<(), CacheError>
    where
        T: Serialize,
    {
        self.set_record_with_ttl(table_name, id, value, self.config.default_ttl).await
    }
    ...
    pub async fn get_query<T>(&self, table_name: &str, query_hash: &str) -> Result<Option<Vec<T>>, CacheError>
    where
        T: for<'de> Deserialize<'de>,
    {
        let cache_key = self.build_query_key(table_name, query_hash);
        let mut conn = self.get_connection().await?;
    ...
    pub async fn set_query<T>(&self, table_name: &str, query_hash: &str, results: &Vec<T>) -> Result<(), CacheError>
    where
        T: Serialize,
    {
        self.set_query_with_ttl(table_name, query_hash, results, self.config.default_ttl).await
    }
    ...
    pub async fn delete_record(&self, table_name: &str, id: &str) -> Result<bool, CacheError> {
        let cache_key = self.build_record_key(table_name, id);
        let mut conn = self.get_connection().await?;
    ...
    pub async fn delete_records(&self, table_name: &str, ids: Vec<String>) -> Result<i32, CacheError> {
        if ids.is_empty() {
            return Ok(0);
        }
        ...
        let cache_keys: Vec<String> = ids
            .into_iter()
            .map(|id| self.build_record_key(table_name, &id))
            .collect();
    ...
    pub async fn invalidate_queries(&self, table_name: &str) -> Result<i32, CacheError> {
        let pattern = format!("{}:{}:query:*", self.config.key_prefix, table_name);
        let mut conn = self.get_connection().await?;
    ...
    pub async fn invalidate_table(&self, table_name: &str) -> Result<i32, CacheError> {
        let record_pattern = format!("{}:{}:record:*", self.config.key_prefix, table_name);
        let query_pattern = format!("{}:{}:query:*", self.config.key_prefix, table_name);
        let mut conn = self.get_connection().await?;
    ...
    pub async fn record_exists(&self, table_name: &str, id: &str) -> Result<bool, CacheError> {
        let cache_key = self.build_record_key(table_name, id);
        let mut conn = self.get_connection().await?;
    ...
    pub async fn record_ttl(&self, table_name: &str, id: &str) -> Result<i64, CacheError> {
        let cache_key = self.build_record_key(table_name, id);
        let mut conn = self.get_connection().await?;
